...
public class Card extends Touchable {
    final static Model card_model = new G3dModelLoader(new JsonReader()).loadModel(Gdx.files.internal("first_card.g3dj"));
    Material card_3Dtexture;
    ...
    boolean handle;
    boolean is3D;
    TextureAttribute simple_3Dtexture=new TextureAttribute(TextureAttribute.Diffuse,new Texture("card_texture.PNG"));
    ...
    String texture_path = "card_texture.PNG";
    BoundingBox box;
    Sprite sprite;
    ...
    public Card() {
        ...
        instance.transform.rotate(1, 0, 0, 270);
    ...
    public void change_texture(int type) {
        if(type==1){sprite.setTexture(simple_2Dtexture);}
        if(type==2){sprite.setTexture(selected_2Dtexture);}
        if(type==3){instance.materials.get(0).set(simple_3Dtexture);}
        if(type==4){instance.materials.get(0).set(selected_3Dtexture);}
    }
        ...
        Generator<Vector3> generator = new Generator<Vector3>() {
            ...
            protected void run() throws InterruptedException {
                ...
                for (int i = 1; i <= frames; i++) {
                    nowPos.set(distanceXpFrame * i * directionX, distanceYpFrame * i * directionY, distanceZpFrame * i * directionZ);
                    yield(nowPos);
                }
            }
        };
    ...
    public Generator<Pair<Vector2, Float>> doAnimation2D(final Vector2 startPos, final Vector2 endPos, final int frames, final float rotation,String id) {
        Generator<Pair<Vector2, Float>> generator = new Generator<Pair<Vector2, Float>>() {
            ...
            protected void run() throws InterruptedException {
                ...
                float rotatePframe = (float) (rotation * 1.0 / frames);
                float distanceXpFrame = (float) (Math.abs(endPos.x - startPos.x) / frames);
                float distanceYpFrame = (float) (Math.abs(endPos.y - startPos.y) / frames);
                ...
                for (int i = 1; i <= frames; i++) {
                    answ.first.set(distanceXpFrame * i * directionX, distanceYpFrame * i * directionY);
                    yield(answ);
                }
            }
        };
    ...
    public void sprite_doubleTouched() {
        //sprite.setTexture(new Texture(Gdx.files.internal("1K-bark_14_Dif.jpg")));
    }

    public void convertTo3D(Vector3 camera_pos) {
    }

    public void convertTo2D(Vector3 camera_pos) {
        Generator<Vector3> animation3D =doAnimation3D(card_pos,camera_pos,120,"convert3D");
    ...
    public void calculate_card_selected(boolean set) {

    }

    public void animations3Dend(String animation_id) {
        switch (animation_id){
            case("convert3D"):{
    ...
    public void Animations2Dend(){

    }

    public void non_selected() {
        if(is3D) {change_texture(3);}
